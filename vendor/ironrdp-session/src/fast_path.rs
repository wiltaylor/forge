                ...
                .contains(ironrdp_pdu::bitmap::Compression::BITMAP_COMPRESSION)
            {
                if update.bits_per_pixel == 32 {
                    ...
                    match ironrdp_graphics::rle::decompress(
                        ...
                        Ok(RlePixelFormat::Rgb16) => image.apply_rgb16_bitmap(&buf, &blit_rect, update.width)?,
                        Ok(RlePixelFormat::Rgb15) => image.apply_rgb15_bitmap(&buf, &blit_rect, update.width)?,
                        Ok(RlePixelFormat::Rgb24) => image.apply_bgr24_bitmap(&buf, &blit_rect, update.width)?,
                        Ok(RlePixelFormat::Rgb8) => {
                            image.apply_rgb8_with_palette(&buf, &blit_rect, self.palette.colors(), update.width)?
                        }
                ...
                if padded_row_bytes != row_bytes {
                    ...
                    match update.bits_per_pixel {
                        8 => image.apply_rgb8_with_palette(&buf, &blit_rect, self.palette.colors(), update.width)?,
                        15 => image.apply_rgb15_bitmap(&buf, &blit_rect, update.width)?,
                        16 => image.apply_rgb16_bitmap(&buf, &blit_rect, update.width)?,
                        24 => image.apply_bgr24_bitmap(&buf, &blit_rect, update.width)?,
                        32 => image.apply_rgb32_bitmap(&buf, PixelFormat::BgrX32, &blit_rect, update.width)?,
                        _ => {
                    ...
                    match update.bits_per_pixel {
                        ...
                        15 => image.apply_rgb15_bitmap(update.bitmap_data, &blit_rect, update.width)?,
                        16 => image.apply_rgb16_bitmap(update.bitmap_data, &blit_rect, update.width)?,
                        24 => image.apply_bgr24_bitmap(update.bitmap_data, &blit_rect, update.width)?,
