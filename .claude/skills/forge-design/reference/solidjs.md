...
| `ChatToolCall` | `tool {name, status: running\|success\|error, summary?, args?, result?, defaultOpen?, key?, children?}`, `open?`, `onToggle?(open)` | Collapsible tool-call box; string args/result render as code blocks; `children` nest recursively. Expansion is controlled when both `open` and `onToggle` are given; else held by ChatView's registry when `tool.key` is set (nested calls inherit `<parent key>/<index>`); else component-local, seeded from `defaultOpen`. Give a `key` to any transcript whose items are rebuilt on update. |
