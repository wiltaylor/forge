...
pub const REGIONS: &[&str] = &["eu-west-1", "eu-central-1", "us-east-1", "us-west-2", "ap-southeast-2"];
pub const PACKAGES: &[&str] = &["forge-core", "forge-server", "forge-auth", "forge-tui", "forge-tauri", "gallery", "parity"];
...
impl Default for PickersState {
    fn default() -> PickersState {
        ...
        packages.toggle(3);
        PickersState {
            notes: TextareaState::with_value("Deploy notes:\n- rolling restart\n- watch the event bus"),
...
pub fn draw(frame: &mut Frame, area: Rect, ctx: &mut Ctx, t: &Theme, state: &mut PickersState) {
    ...
    if right.height > 2 {
        frame.render_widget(Eyebrow::new("Notes (Textarea)").theme(t), Rect::new(right.x, right.y, right.width, 1));
        frame.render_stateful_widget(
            Textarea::new().placeholder("Notes…").focused(f_notes).theme(t),
            Rect::new(right.x, right.y + 1, right.width.min(44), right.height.saturating_sub(1).min(8)),
            &mut state.notes,
        );
    }
    ...
    if let Some(r) = r_image {
        frame.render_stateful_widget(
            Combobox::new(IMAGES).placeholder("Search images…").focused(f_image).theme(t),
            r,
            &mut state.image,
        );
    }
