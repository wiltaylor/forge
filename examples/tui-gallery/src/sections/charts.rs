...
pub fn draw(frame: &mut Frame, area: Rect, ctx: &mut Ctx) {
    ...
    if let Some(r) = blocks.first().filter(|r| r.height > 2) {
        frame.render_widget(Eyebrow::new("LineChart"), Rect::new(r.x, r.y, r.width, 1));
    ...
    if let Some(r) = blocks.get(1).filter(|r| r.height > 2) {
        frame.render_widget(Eyebrow::new("PieChart"), Rect::new(r.x, r.y, r.width, 1));
        let slices = [
    ...
    if let Some(r) = blocks.get(2).filter(|r| r.height > 2) {
        frame.render_widget(Eyebrow::new("BarChart"), Rect::new(r.x, r.y, r.width, 1));
        let data = [
            ("mon", 32u64),
            ("tue", 41),
            ("wed", 28),
    ...
    if let Some(r) = blocks.get(3).filter(|r| r.height > 2) {
        frame.render_widget(Eyebrow::new("Gantt"), Rect::new(r.x, r.y, r.width, 1));
        let tasks = [
    ...
    if let Some(r) = blocks.get(4).filter(|r| r.height >= 2) {
        let data: Vec<u64> = (0..r.width as u64)
            ...
            .collect();
        frame.render_widget(
            forge_tui::widgets::charts::Sparkline::new(&data),
            Rect::new(r.x, r.y, r.width / 2, 1),
        );
        frame.render_widget(
            Legend::new(&["requests", "errors", "success", "queued", "info"]),
