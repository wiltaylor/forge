...
///
/// Read with [`Theme::text`]. Named `TextRole` rather than `Text` so it never
/// collides with `ratatui::text::Text` in a widget file.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum TextRole {
...
impl Theme {
    ...
    pub fn surface(&self, s: Surface) -> Color {
        match s {
    ...
    pub fn text(&self, r: TextRole) -> Color {
        match r {
