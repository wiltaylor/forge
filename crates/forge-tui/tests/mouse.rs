...
fn checkbox_and_toggle_click() {
    ...
    Checkbox::new("agree").theme(&t).render(Rect::new(0, 0, 20, 1), &mut buf, &mut cb);
...
fn input_click_places_cursor() {
    ...
    let mut buf = Buffer::empty(Rect::new(0, 0, 20, 1));
    Input::new().focused(true).theme(&t).render(Rect::new(0, 0, 20, 1), &mut buf, &mut s);
...
fn listbox_click_selects_and_wheel_moves() {
    ...
    ListBox::new(&items).theme(&t).render(Rect::new(0, 0, 20, 4), &mut buf, &mut s);
...
fn table_header_click_sorts_row_click_moves_cursor() {
    ...
    Table::new(&columns, &rows).theme(&t).render(Rect::new(0, 0, 30, 5), &mut buf, &mut s);
...
fn tabs_and_pagination_click() {
    ...
    Tabs::new(&["one", "two", "three"]).theme(&t).render(Rect::new(0, 0, 40, 2), &mut buf, &mut tabs);
    ...
    Pagination::new().theme(&t).render(Rect::new(0, 0, 40, 1), &mut buf, &mut pages);
    // '›' next arrow.
...
fn slider_click_snaps_to_step() {
    ...
    Slider::new().show_value(false).theme(&t).render(Rect::new(0, 0, 25, 1), &mut buf, &mut s);
...
fn select_click_open_choose_and_click_away() {
    ...
    Select::new(&items).theme(&t).render(field, &mut buf, &mut s);
...
fn tree_click_moves_then_toggles() {
    const KIDS: [TreeNode<'static>; 1] = [TreeNode { label: "leaf", children: &[] }];
    let roots = [TreeNode::branch("root", &KIDS)];
    let t = Theme::dark();
    let mut s = TreeState::new();
    let mut buf = Buffer::empty(Rect::new(0, 0, 20, 5));
    Tree::new(&roots).theme(&t).render(Rect::new(0, 0, 20, 5), &mut buf, &mut s);
...
fn menu_hover_highlights_click_submits_away_cancels() {
    ...
    let mut buf = Buffer::empty(area);
    DropdownMenu::new(&entries, Rect::new(0, 0, 1, 1)).theme(&t).render(area, &mut buf, &mut s);
...
fn shell_nav_click_switches_section() {
    ...
    AppShell::new("T", &sections).subtitle("sub").theme(&t).render(
        Rect::new(0, 0, 80, 20),
        &mut buf,
        &mut s,
    );
...
fn logs_wheel_unpins_follow() {
    let t = Theme::dark();
    let lines: Vec<LogLine> = (0..30).map(|i| LogLine::new(Level::Info, format!("l{i}"))).collect();
...
fn kanban_click_focuses_then_submits() {
    ...
    Kanban::new(&cols).theme(&t).render(Rect::new(0, 0, 30, 10), &mut buf, &mut s);
