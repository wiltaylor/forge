...
fn listbox_click_selects_and_wheel_moves() {
    ...
    ListBox::new(&items).render(Rect::new(0, 0, 20, 4), &mut buf, &mut s);
...
fn table_header_click_sorts_row_click_moves_cursor() {
    ...
    Table::new(&columns, &rows).render(Rect::new(0, 0, 30, 5), &mut buf, &mut s);
...
fn select_click_open_choose_and_click_away() {
    ...
    Select::new(&items).render(field, &mut buf, &mut s);
...
fn tree_click_moves_then_toggles() {
    ...
    Tree::new(&roots).render(Rect::new(0, 0, 20, 5), &mut buf, &mut s);
...
fn menu_hover_highlights_click_submits_away_cancels() {
    ...
    let mut buf = Buffer::empty(area);
    DropdownMenu::new(&entries, Rect::new(0, 0, 1, 1)).render(area, &mut buf, &mut s);
...
fn logs_wheel_unpins_follow() {
    ...
    let mut buf = Buffer::empty(area);
    Logs::new(&lines).render(area, &mut buf, &mut s);
    assert!(s.follow);
...
fn kanban_click_focuses_then_submits() {
    ...
    Kanban::new(&cols).render(Rect::new(0, 0, 30, 10), &mut buf, &mut s);
