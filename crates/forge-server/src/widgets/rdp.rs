...
//!
//! The server runs a full IronRDP client — TLS upgrade (dev-permissive
//! verifier, see docs/widgets-protocol.md), CredSSP/NLA, bitmap decode into
//! an RGBA [`DecodedImage`] — and streams dirty rects as
//! [`super::proto::encode_rect`] frames. Browser input becomes FastPath
...
async fn session(mut socket: WebSocket, config: Arc<DesktopConfig>) {
    // The first frame must be a valid `connect` with full credentials.
            ...
            Message::Text(text) => match serde_json::from_str::<DesktopClientMsg>(&text) {
                Ok(DesktopClientMsg::Connect {
                    ...
                    let (Some(host), Some(username), Some(password)) = (host, username, password)
                        ...
                        return fail(socket, "rdp requires host, username and password").await;
                    };
    ...
    let (connection_result, framed) =
        match tokio::time::timeout(CONNECT_TIMEOUT, connect(&host, port, username, password)).await
        {
            ...
            Err(_) => return fail(socket, format!("rdp connect to {host}:{port} timed out")).await,
        };

    run(&mut socket, connection_result, framed).await;
...
/// browser input → FastPath input PDUs.
async fn run(
    ...
    if send_ctrl(socket, &ready).await.is_err() {
        return;
    }
            ...
            pdu = framed.read_pdu() => {
                ...
                let outputs = match stage.process(&mut image, action, &payload) {
                    ...
                    Err(e) => {
                        ...
                        let _ = send_ctrl(socket, &msg).await;
                        return;
                    }
                };
                if !forward_outputs(socket, &mut framed, &image, outputs).await {
                    return;
                }
            }
            msg = socket.recv() => {
...
/// Translate one browser control message into input-state operations.
