//! In-process event bus fanned out over SSE (`/api/events`) and WebSocket
//! (`/api/ws`). Live-telemetry semantics: bounded buffer, slow consumers lag.
...
///
/// SSE and WS subscribers fan out from the same
/// [`tokio::sync::broadcast`] channel; publishing when nobody listens is fine.
#[derive(Clone)]
pub struct EventBus {
