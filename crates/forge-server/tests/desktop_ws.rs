//! /api/desktop/{vnc,rdp} session behaviour: config gating, connect
//! failures, and live round-trips behind #[ignore]. Route mounting + auth
//! follow the same wiring as /api/term (widgets_ws.rs), so these routers run
//! auth-disabled.
...
mod rdp {
    ...
    async fn connect_rdp_ws(config: DesktopConfig) -> Ws {
        let router = ForgeApp::new("t").with_rdp_config(config).router();
        let addr = spawn(router).await;
        let (ws, _) = connect_async(format!("ws://{addr}/api/desktop/rdp"))
            .await
            .expect("ws connect");
        ws
    }
    ...
    #[tokio::test]
    async fn missing_credentials_yields_error() {
        let mut ws = connect_rdp_ws(DesktopConfig::default()).await;
        send_json(&mut ws, json!({"type":"connect","host":"h"})).await;
        let msg = next_ctrl(&mut ws).await;
        assert_eq!(msg["type"], "error");
    ...
    #[tokio::test]
    async fn host_not_in_allowlist_yields_error() {
        let mut ws = connect_rdp_ws(DesktopConfig {
            allow_hosts: Some(vec!["allowed.example".into()]),
        })
        .await;
        send_json(
            &mut ws,
            json!({"type":"connect","host":"other.example","username":"u","password":"p"}),
        )
        .await;
        let msg = next_ctrl(&mut ws).await;
        assert_eq!(msg["type"], "error");
    ...
    async fn refused_target_yields_error() {
        ...
        send_json(
            &mut ws,
            json!({"type":"connect","host":"127.0.0.1","port":9,"username":"u","password":"p"}),
        )
        .await;
        let msg = next_ctrl(&mut ws).await;
        assert_eq!(msg["type"], "error");
    ...
    /// Live RDP basic-connect. Run with:
    /// `FORGE_TEST_RDP_ADDR=host:port FORGE_TEST_RDP_USER=u FORGE_TEST_RDP_PASS=p \
    ///  cargo test -p forge-server --features widgets -- --ignored rdp_live`
    #[tokio::test]
    #[ignore = "needs FORGE_TEST_RDP_ADDR + FORGE_TEST_RDP_USER + FORGE_TEST_RDP_PASS"]
    async fn rdp_live_basic_connect() {
        let addr = std::env::var("FORGE_TEST_RDP_ADDR").expect("FORGE_TEST_RDP_ADDR");
        let user = std::env::var("FORGE_TEST_RDP_USER").expect("FORGE_TEST_RDP_USER");
        let pass = std::env::var("FORGE_TEST_RDP_PASS").expect("FORGE_TEST_RDP_PASS");
        let (host, port) = match addr.split_once(':') {
            Some((h, p)) => (h.to_string(), p.parse::<u16>().expect("port")),
            None => (addr, 3389),
        };
        ...
        send_json(
            &mut ws,
            json!({"type":"connect","host":host,"port":port,"username":user,"password":pass}),
        )
        .await;

        let ready = next_ctrl(&mut ws).await;
        assert_eq!(ready["type"], "ready", "rdp connect failed: {ready}");
        assert!(ready["width"].as_u64().unwrap() > 0);

        // Expect at least one valid rect frame (login screen or desktop).
        let deadline = tokio::time::Instant::now() + WAIT;
        loop {
            let msg = tokio::time::timeout_at(deadline, ws.next())
                .await
