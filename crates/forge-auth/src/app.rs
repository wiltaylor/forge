...
pub fn build_router(state: SharedState, forge_app: ForgeApp) -> Result<Router, AppError> {
    let app = add_dev_routes(forge_app)
        ...
        .route("/.well-known/openid-configuration", get(oidc::discovery::openid_configuration))
        ...
        .route("/oauth2/userinfo", get(oidc::userinfo::userinfo).post(oidc::userinfo::userinfo))
        .route("/oauth2/revoke", post(oidc::revoke::revoke))
        .route("/oauth2/introspect", post(oidc::introspect::introspect))
        .route("/oauth2/logout", get(oidc::end_session::end_session).post(oidc::end_session::end_session))
        ...
        .route("/api/login/upstream/{slug}", get(crate::upstream::start_upstream_login))
        .route("/api/callback/{slug}", get(crate::upstream::upstream_callback))
        .route("/api/session", get(login::session_info))
        .route("/api/logout", post(login::logout))
        .route("/api/consent/{id}", get(oidc::authorize::consent_info).post(oidc::authorize::consent_decide))
        // --- self-service ---
        ...
        .route(
            "/api/admin/users/{id}",
            get(admin::users::get).put(admin::users::update).delete(admin::users::delete),
        )
        .route("/api/admin/users/{id}/password", put(admin::users::set_password))
        .route("/api/admin/users/{id}/roles", put(admin::users::set_user_roles))
        .route("/api/admin/roles", get(admin::roles::list).post(admin::roles::create))
        .route("/api/admin/roles/{id}", delete(admin::roles::delete))
        .route("/api/admin/clients", get(admin::clients::list).post(admin::clients::create))
        .route(
            "/api/admin/clients/{id}",
            get(admin::clients::get).put(admin::clients::update).delete(admin::clients::delete),
        )
        .route("/api/admin/clients/{id}/secret", post(admin::clients::regenerate_secret))
        .route("/api/admin/providers", get(admin::providers::list).post(admin::providers::upsert))
        .route(
            "/api/admin/providers/{id}",
            get(admin::providers::get).delete(admin::providers::delete),
        )
        .route("/api/admin/providers/{id}/test", post(admin::providers::test))
